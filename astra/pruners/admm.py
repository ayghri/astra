...
Supports two modes:
  ...
  - Error-correcting: minimize ||W X_pruned - W0 X_dense||^2 under sparsity
    (corrects for upstream pruning error)
"""
...
def admm_prune(
    ...
    Args:
        ...
        C_target: [M, K] target correlation. None = W0 @ H (standard mode).
            For error-correcting: W0 @ (X_dense^T @ X_pruned / N).
        ...
        dtype: internal computation dtype.
    ...
    rho = H.diag().diag()
    mat_A = torch.linalg.solve(H + rho, torch.eye(K, device=device))
    mat_C = C_target.mm(mat_A)

    # Cast to working dtype
    mat_A = mat_A.to(dtype)
    mat_C = mat_C.to(dtype)
    rho_d = H.diag().diag().to(dtype)
    H_diag = H.diag().diag().to(dtype)
    H_d = H.to(dtype)
    C_d = C_target.to(dtype)

    Z = View.from_existing(torch.zeros(M, K, device=device, dtype=dtype))
    W = Z.param.clone()
    U = torch.zeros_like(W)
    rho_cond = torch.ones_like(W) @ rho_d
    ...
    for _ in rng:
        W = mat_C + (Z.param - U) @ rho_d @ mat_A
        Z.param.copy_(W + U)

        # Gradient with diagonal removed
        psi_pre = W @ H_d - C_d
